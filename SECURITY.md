...
- canquery exposes an anonymous read API and ingests **only** files already listed
  in the upstream catalogue (never arbitrary user-supplied URLs). Reports about the
  ingest pipeline, the query/filter grammar, SQL handling, or resource-exhaustion
  limits are especially welcome.
